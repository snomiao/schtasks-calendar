...
describe("command exec", () => {
  const cwdright = process.cwd().endsWith("schcal");
  it.skip("open long url", async () => {
    if (!cwdright) return;
    const url =
    ...
    await commandFileWrapper(url);
  });
  it.skip("run a complex cmd commands", async () => {
    if (!cwdright) return;
    // eslint-disable-next-line max-len
  ...
  it.skip("open local file explorer by absolute path", async () => {
    if (!cwdright) return;
    await commandFileWrapper("C:/Windows/");
  });
  it.skip("start browser", async () => {
    if (!cwdright) return;
    await commandFileWrapper("chrome");
  });
});

async function commandFileWrapper(command: string) {
