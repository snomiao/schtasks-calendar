...
async function readConfig(argv) {
    const configPaths = [argv.config, env.CONFIG, 'config.yaml', process.cwd() + '/config.yaml']
...
async function generateSchtasksCreationObjects(config) {
    ...
    if (!ICS_URLS?.length) process.exit(1)
...
async function 多日历将行动作列抓取(ics_urls, cacheTimeout, httpProxy, FORWARD_DAYS) {
    return (await Promise.all(ics_urls.map(async (ics_url) =>
        await 日历将行动作列抓取(ics_url, cacheTimeout, httpProxy, FORWARD_DAYS)))).flat(1);
}
